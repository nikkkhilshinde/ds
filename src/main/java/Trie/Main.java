...
//        trainTree();
        ...
        do{
            ...
            for(int i = 0; i < input.length(); i++){
                tempNode =  insert(input.charAt(i), tempNode);
            }
            System.out.println(
...
//        persistNodeInFile(rootNode);
        ...
        try {
            ...
            wordMeaningMap.forEach((key, value) -> {
                ...
                for(int i = 0; i < key.length(); i++){
                    tempNode =  insert(key.charAt(i), tempNode);
                }
    ...
    /**
     * It inserts a single character into tree and returns
     * the inserted node
     *
     * @param  charAt   Character to be inserted into tree
     * @param  tempNode Root node of the tree
     * @return          Returns the inserted node
     */
    static TrieNode insert(char charAt, TrieNode tempNode) {
        if(tempNode.data == 0){
            System.out.println("Its root node, search in children");
        }
        return searchInChildNodes(charAt,tempNode.nodes);
    }
